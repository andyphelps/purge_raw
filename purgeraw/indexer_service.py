...
SINGLE_IMAGE_INDEX_RE = re.compile("^.*?([0-9]{3,6})\\..{3,4}$")
RANGE_IMAGE_INDEX_RE = re.compile("^.*?([0-9]{3,6})-([0-9]{3,6})\\..{3,4}$")


def indexer(path: str) -> List[Tuple[str, str]]:

    range_matcher: Optional[Match[str]] = RANGE_IMAGE_INDEX_RE.match(path)
    ...
    single_matcher: Optional[Match[str]] = SINGLE_IMAGE_INDEX_RE.match(path)
